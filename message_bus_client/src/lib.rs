...
impl ZmqMessageBusClient {
    ...
    >(
        ...
        message_processor: T,
        published_message_processor: U,
    ) -> ZmqMessageBusClient {
        let identity = configurations.get_string("identity").unwrap();
        let channels_strings = configurations.get_vec("subscription_channels").unwrap();
        let message_bus_address_for_pubs = configurations.get_string("address_for_pubs").unwrap();
        let message_bus_address_for_subs = configurations.get_string("address_for_subs").unwrap();
        let message_bus_address_for_router =
            configurations.get_string("address_for_router").unwrap();
        ...
        let requester = req_client::RequestClient::new(
        ...
        let _subscriber_handle = thread::spawn({
            move || {
                ...
                let subscriber =
                    subscriber::Subscriber::new(channels, &message_bus_address_for_subs, false);
                loop {
...
impl MessageBusClient for ZmqMessageBusClient {
    fn send_request(&self, destination: &str, data: &str) -> Result<String, String> {
        self.requester
            .send_request(&format!("{}{}", destination, "_replier"), data)
    }
    ...
    fn subscribe_channel(&self, channel: &str) {
        //TODO
        println!(
            "subscribe_channel not implemented. {} not subscribed.",
            channel
        );
    }
}
